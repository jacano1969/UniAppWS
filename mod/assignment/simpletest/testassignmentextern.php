...
require_once(UNIAPP_ROOT . '/mod/assignment/externallib.php');

global $DB;
Mock :: generatePartial(get_class($DB), 'assignmentMockDB', array('get_records',  'insert_record', 'update_record', 'get_record_sql','get_record'));
...
class assignmentexternal_test extends UnitTestCase {
  ...
  function setUp() {
    ...
    public function test_get_submission_by_assigid() {
        ...
        $DB->setReturnValueAt(0, 'get_record', (object) $assig);
        $DB->setReturnValueAt(1, 'get_record', (object) $submi);
        $DB->setReturnValueAt(0, 'get_record_sql', (object) array('id' => 1));
        $DB->setReturnValueAt(0, 'insert_record_raw', true);

        $params = 1;
        $return = local_uniappws_assignment :: get_submission_by_assigid($params);
    ...
    public function test_get_submission_files() {
        ...
        $DB->setReturnValueAt(0, 'get_record', (object) array('id' => 3));//subid
        $DB->setReturnValueAt(0, 'get_record', (object) $context); //get_context_instance
        $DB->setReturnValueAt(0, 'get_records', $files);
        $return = local_uniappws_assignment :: get_submission_files($params['assigid'], 0, 10);
